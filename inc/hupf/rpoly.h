...
#include <time.h>
#include <hupf/Complex.h>
#include <hupf/Polynomial.h>

namespace LibHUPF
{


std::vector<Complex> calcRoots(Polynomial n);
...
public:
  ...
  std::vector<Complex> calcRoots(Polynomial n)
  {
    ...
    int deg = n.degree;
    double *op=(double*)malloc(sizeof(double)*(deg+1));
    for(int i=0; i<=deg; i++)
    {
      op[i]=n.coefficient[deg-i];
    }
    double *zeror=(double*)malloc(sizeof(double)*(deg+1));
    double *zeroi=(double*)malloc(sizeof(double)*(deg+1));
    int inf[100];
    int rootsFound = rpoly(op,deg,zeror,zeroi,inf);
    for(int i=0; i<rootsFound; i++)
    {
      results.push_back(Complex(zeror[i],zeroi[i]));
    }
    free(zeror);
    free(zeroi);
    free(op);
